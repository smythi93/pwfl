"""
Consistency checks for generated PWFL report artifacts.

The functions in this module scan report JSON files and highlight projects that
require manual investigation (errors, missing buggy tests, or empty data).
"""
...
def check_cg(msg, pattern, need_investigation_file, directory=None):
    """
    Scan report files matching a naming pattern and aggregate issues.

    :param msg: Log message prefix describing the current check.
    :type msg: str
    :param pattern: Compiled regex used to select relevant report files.
    :type pattern: re.Pattern[str]
    :param need_investigation_file: Output JSON file path for issue details.
    :type need_investigation_file: str
    :param directory: Directory containing report files.
        ...
        if match:
            ...
            for identifier in report:
                ...
                if (
                    ...
                    # Preserve reason-level details so failures can be triaged later.
...
def check_cg_build(directory=None):
    """
    Check call-graph build reports.

    :param directory: Optional report directory override.
    :type directory: str | None
    :returns: None
    """
    check_cg(
...
def check_cg_events(directory=None):
    """
    Check call-graph event collection reports.
...
def check_cg_pr(directory=None):
    """
    Check PRFL/PageRank reports built from call graphs.
...
def check_events(directory=None):
    """
    Check baseline event collection reports.
...
def check_tcp(directory=None):
    """
    Check TCP event collection reports.
