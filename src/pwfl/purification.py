"""
Test-case purification integration for the PWFL pipeline.

This module orchestrates TCP steps end-to-end: test purification,
instrumentation, event collection, analyzer generation, and rank refinement.
"""
...
from pyurify.purification import purify_tests, rank_refinement
...
def create_config(
    ...
    """
    Create SFLKit instrumentation configuration for TCP event collection.

    :param project: Subject metadata.
    :param src: Source checkout directory.
    :param dst: Instrumented working directory.
    :param metrics: Optional metrics string.
    :param events_path: Optional explicit event output path.
    :param mapping: Optional mapping output path.
    :param include_suffix: Use tests4py suffix layout when ``True``.
    :returns: Generated SFLKit config.
    :rtype: sflkit.Config
    """
...
def sflkit_instrument(
    ...
    """
    Instrument one purified subject checkout.

    :param project: Subject metadata.
    :param src: Source directory.
    :param dst: Destination directory for instrumented files.
    :param mapping: Optional mapping path.
    :param report: Optional report object to fill.
    :returns: Instrumentation report with success/error state.
...
def sflkit_unittest(
    ...
    """
    Execute relevant tests against an instrumented checkout.

    :param work_dir: Instrumented project directory.
    :param project: Subject metadata.
    :param output: Event output directory.
    :returns: Event collection report.
    :rtype: tests4py.sfl.SFLEventsReport
    """
...
def parse_test_id(test_id: str) -> tuple:
    """
    Parse a pytest test id into normalized components.

    :param test_id: Raw test id, e.g. ``file.py::Class::test[param]``.
    :type test_id: str
    :returns: Tuple ``(file, class_name, test_name, param_suffix)`` where
        ``class_name`` and ``param_suffix`` may be ``None``.
...
def purify(
    ...
    """
    Run purification for a single project and store metadata in report.

    :param project: Subject metadata.
    :param identifier: Stable project identifier.
    :param report: Mutable report dictionary.
    :param enable_slicing: Enable dynamic slicing in purification.
    :type enable_slicing: bool
    :returns: Mapping from original failing tests to purified test files, or
        ``None`` on failure.
    :rtype: dict[str, list[tuple[pathlib.Path, str | None]]] | None
    """
...
def update_project_purified(
    ...
    """
    Replace project test references with purified test artifacts.

    :param project: Subject metadata to mutate in place.
    :param identifier: Stable project identifier.
    :param purified_mapping: Mapping returned by :func:`purify`.
    :returns: None
    """
    purified_tests_dir = Path("tmp", f"{identifier}_purified")
...
def build(
    ...
    """
    Instrument the purified checkout and write its event mapping.
...
def collect(
    ...
    """
    Run tests on instrumented purified code and collect TCP events.

    :returns: Event output path.
    :rtype: pathlib.Path
    """
    sfl_path = Path("tmp", f"sfl_{identifier}_tcp")
    events_base = Path(
...
def get_tcp_events(
    ...
    """
    Collect TCP events for selected projects.
    ...
    :param enable_slicing: Enable dynamic slicing during purification.
    :type enable_slicing: bool
    :returns: None
    """
...
def tcp_analyze_project(
    ...
    """
    Build one TCP analyzer variant and export purified spectra.

    :param project: Subject metadata.
    :param analysis_file: Output analyzer path.
    :param report: Mutable report dictionary.
    :param suffix: Variant suffix.
    :param identifier: Stable project identifier.
    :param model_class: Optional proximity model class.
    :returns: Constructed analyzer instance.
    :rtype: sflkit.Analyzer
    """
    ...
    for event_file in failing:
        ...
        for ao in analyzer.get_analysis_by_type(AnalysisType.LINE):
            if event_file in ao.hits:
                if any(
                    ao.hits[event_file][thread_id] > 0
...
def tcp_analyze(project_name, bug_id=None, start=None, end=None):
    """
    Generate TCP analyzers for all selected projects.
...
def tcp_get_results_for_type(
    ...
    """
    Evaluate TCP suggestions for one analysis type and metric family.

    :param clean: If ``True``, skip rank refinement and use baseline suggestions.
    :type clean: bool
...
def tcp_evaluate(project_name, bug_id, start=None, end=None, clean=False):
    """
    Evaluate TCP analyzers (with optional clean mode).

    :param project_name: Project identifier.
    :param bug_id: Optional single bug id.
    :param start: Optional lower bound bug id.
    :param end: Optional upper bound bug id.
    :param clean: Disable rank refinement when ``True``.
    :returns: None
    """
